...
校鮮集原型可把 Falcon 1／iMin F1 作為「智販機／取貨設備／收銀終端」展示：掃描 1D／2D 取貨碼、確認訂單、核對福利折抵、列印取貨單／QR／發票樣張、回報設備事件。正式版本仍需人工確認實際型號、Android／韌體版本、SDK、設備管理、網路、支付、電子發票服務商、稅籍與字軌、庫存扣減、開櫃協議、資安與維運 owner。所有硬體操作應透過 `IHardwareDeviceProvider`／Printer／Invoice Adapter，並在失敗時 fail-closed，不直接標記付款成功、發票完成、取貨完成或重複扣款。
