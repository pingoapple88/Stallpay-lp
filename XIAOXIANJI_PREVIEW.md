...
頁面包含多校與校點管理、會員分群、商品與團購檔期、訂單與配送／取貨、對帳中心，以及 Order AI、MerchLoop、StallPay 的 LINE 互動概念展示。頁面不呼叫外部 API、不收集會員資料、不建立訂單，也不保存付款資訊。
...
HTML parser、inline JavaScript `node --check`、禁用詞檢查與瀏覽器桌面版驗證均已執行。瀏覽器驗證確認首屏、平台 tab、FAQ 展開／收合、英文／繁體中文切換與展示邊界提醒可用；完整紀錄見 `QA_BROWSER_NOTES.md`。
